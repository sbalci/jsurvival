...
> **Not yet released.** Time-dependent covariates are **not available in
> jsurvival 1.0.4**. The options this article describes —
> `use_time_dependent`, `td_format`, `time_dep_vars`, `change_times`,
> `start_time_var`, `stop_time_var` and `td_suffix_pattern` — are
> present in `jamovi/multisurvival.a.yaml` but commented out, under a
> heading that reads *“Time-Dependent Covariates (EXPERIMENTAL - will be
> implemented later)”*. They are therefore absent from the compiled
> analysis and from the
> [`multisurvival()`](https://www.serdarbalci.com/jsurvival/reference/multisurvival.md)
> R function, and passing any of them raises `unused argument`. The same
> applies to the frailty and spline options this article also uses
> (`use_frailty`, `frailty_var`, `frailty_distribution`, `use_splines`,
> `spline_vars`, `spline_df`, `spline_type`) and to the model-selection
> options (`use_modelSelection`, `modelSelection`, `selectionCriteria`)
> — 17 of the options shown below in total.
>
> This article is kept as the design specification for that future
> release. The statistical background in it is sound and worth reading;
> the code is not yet runnable. Every code chunk is set to
> `eval = FALSE`, so nothing here is executed when the article is built.
> For what `multisurvival` can do today — including stratification,
> adjusted survival curves, risk scores, nomograms and person-time
> analysis — see
> [`?multisurvival`](https://www.serdarbalci.com/jsurvival/reference/multisurvival.md)
> and the other multisurvival articles.

## Introduction

Time-dependent covariates are variables that change their values over
the course of follow-up time. In clinical research, many important
variables can change during the study period, such as:
...
The `multisurvival` function in ClinicoPath now supports time-dependent
covariates in both wide and long data formats, making it easier to
analyze survival data where covariates change over time.

## Data Format Options

### Wide Format Data

In wide format, each subject has one row, with time-dependent variables
represented as separate columns for different time points.
...
**Wide Format Advantages:** - One row per patient (easier to
understand) - Common format in clinical databases - Easy to create from
existing data

**Wide Format Requirements:** - Baseline variables: `variable_baseline`
or just `variable` - Time-specific variables: `variable_t6`,
`variable_t12`, etc. - Change times specified in options

### Long Format Data

In long format, each subject can have multiple rows, with one row for
each time interval where covariates remain constant.
...
**Long Format Advantages:** - More flexible for complex time patterns -
Standard format for survival analysis packages - Can handle irregular
time intervals

**Long Format Requirements:** - `tstart` and `tstop` variables for time
intervals - Status = 1 only in final interval if event occurs -
...
1.  **Hazard Ratios**: Represent the instantaneous risk at any given
    time, accounting for the current value of time-dependent variables.

2.  **Model Fit**: Time-dependent models often fit better than static
    models but require more data and assumptions.

3.  **Causality**: Time-dependent covariates help address time-varying
    confounding but don’t guarantee causal interpretation.

### Common Pitfalls

1.  **Immortal Time Bias**: Ensure time-dependent variables are properly
    aligned with risk periods.

2.  **Missing Data**: Handle missing time-dependent measurements
    appropriately.

3.  **Measurement Timing**: Consider whether variable changes are known
    at the time of measurement or retrospectively.
...
Time-dependent covariates in `multisurvival` provide a powerful tool for
analyzing survival data where important variables change over time. The
support for both wide and long data formats makes this functionality
accessible to researchers working with different data structures.
...
For most clinical applications, wide format provides the most intuitive
approach, while long format offers maximum flexibility for complex time
patterns.
